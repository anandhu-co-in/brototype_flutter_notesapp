...
class NotesDB extends ApiCalls {
  ...
  NotesDB._internal() {
    this.dio.options =
        BaseOptions(responseType: ResponseType.plain, baseUrl: url.baseUrl);
  }
  ...
  Future<List<NoteModel>> getAllNotes() async {
    ...
    if (result.data != null) {
      final notesResponse = AllNotesModel.fromJson(
  ...
  Future<NoteModel?> updateNote(NoteModel value) async {
    try {
      ...
      final resultAsJson = jsonDecode(result.data);
      final updatedNote =
          NoteModel.fromJson(resultAsJson as Map<String, dynamic>);
      final index = noteListNotifier.value
          .indexWhere((element) => element.id == updatedNote.id);
